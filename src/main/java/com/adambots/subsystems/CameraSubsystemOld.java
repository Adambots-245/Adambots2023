...
public class CameraSubsystemOld extends SubsystemBase {
    ...
    public CameraSubsystemOld(Solenoid ringLight, ReflectivePipeline reflectivePipeline) {
    ...
    public void init() {
        ...
        downDetectionCamera = CameraServer.startAutomaticCapture(1);
        reflectivetDetectionCamera = CameraServer.startAutomaticCapture(0);
        // Set the resolution
        ...
        processedOutputStreamHub = CameraServer.putVideo("CameraHub-Output", VisionConstants.kFrameWidth, VisionConstants.kFrameHeight);
        processedOutputStreamHub.setVideoMode(PixelFormat.kGray, VisionConstants.kFrameWidth, VisionConstants.kFrameHeight, VisionConstants.kDriverStationFramesPerSec);
        processedOutputStreamHub.setFPS(VisionConstants.kDriverStationFramesPerSec);
        processedOutputStreamHub.setPixelFormat(PixelFormat.kGray);

        reflectiveMat = new Mat();
        //FindAprilTag();
    ...
    public void run() {
      ...
      while (!Thread.interrupted()) {
          ...
          try (AprilTagReader tagReader = new AprilTagReader()){
            tagReader.detect(reflectiveMat);
           ...
           // var id2 = tagReader.getId(1);
            ...
            for (var i = 0; i <= 3; i++) {
                var j = (i + 1) % 4; 
                Imgproc.line(reflectiveMat, corners[i], corners[j], outlineColor, 2);
            }
          } catch (Exception e){
            // ignore
          }
        ...
        processedOutputStreamHub.putFrame(reflectiveMat);
      }
